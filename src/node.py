...
# Basic imports
import roslib
roslib.load_manifest('particle_filter')
import rospy
import tf
...
from functions import publish_lines, get_map, get_particle_msgs, yaw_from_quaternion, angle_wrap
...
class LocalizationNode(object):
    ...
    def laser_callback(self, msg):
        ...
        if self.lines is not None:
            ...
            # Publish results
            publish_lines(self.lines, self.pub_lines, frame=msg.header.frame_id,
                      time=msg.header.stamp, ns='scan_lines', color=(1,0,0))
            publish_lines(self.lines, self.pub_lines, frame='/mean_particle',
                      time=msg.header.stamp, ns='scan_lines_mean', color=(0,0,1))
